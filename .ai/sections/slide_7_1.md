...
Same horizontal leash axis as Slide 2.3 — same visual style, same endpoint labels.

Three labeled rows below the axis, each with a single dot positioned along the leash:

| Row | Label | Dot position |
|---|---|---|
| 1 | "us, two years ago" | far left |
| 2 | "us, today" | far right |
| 3 | "the people we build software for, today" | far left — **vertically aligned with Row 1** |

*Optional Row 4:* just "?" with two empty dots at unknown positions — implicit "where this is going."

Monochrome. Same dot size and style across rows.

> The **vertical alignment** between Row 1 and Row 3 is the single most important visual detail on the slide. The audience should see — without you saying it — that "the people we build software for are where we were two years ago."
