...
**Title bar:** `P2 — Sub-agents | service composition / saga`

---

## Diagram

P1's diagram with new elements added.

**Orchestrator (top):** The top box is relabeled **COMPLAINT_HANDLER_AGENT** and visually promoted — larger, distinct fill, with "(orchestrator)" underneath. This is the only renaming in the whole construction arc; it reflects a genuine role change.

**New middle row:** Three sub-agent boxes, equal size:
- `ACCOUNT_AGENT`
- `FRAUD_AGENT`
- `RESPONSE_AGENT`

**Tools migrate under their owning sub-agents:**
- `ACCOUNT_AGENT` → `get_account`, `get_transactions`
- `FRAUD_AGENT` → `check_fraud_patterns`
- `RESPONSE_AGENT` → `generate_pdf` *(NEW — mark with a badge, asterisk, or different border)*
  - `RESPONSE_AGENT` also emits a 📄 PDF artifact, visually emphasized as the concrete output.

**`hold_card` stays directly under the orchestrator**, at the same vertical level as the sub-agent row — NOT under any sub-agent. This positioning is the visible setup for P3.

The old tools (`get_account`, `get_transactions`, `check_fraud_patterns`, `hold_card`) should render in exactly the same visual style as P1 — the audience eye should recognize them.
...
Bold/color `sub_agents=[...]` — same visual treatment as `tools=[...]` in P1.

*Optional:* if your tooling supports a strikethrough-on-entry animation for the three tools that moved, it would beautifully visualize the migration. Don't force it if it's a hassle.
