...
**Title bar:** `P4 — Sessions & memory | request scope / database`

---

## Diagram

P3's diagram with two new components added on the **right side**.

**STATE box (upper right):**
- Label: `STATE — this session`
- Light/ephemeral visual style
- Example keys inside: `draft_response`, `customer_id`, `...`
- Connected to the orchestrator with a **dashed line** — dashed because it's not a tool call, just a place values get written and read.

**MEMORY_SERVICE box (lower right):**
- Label: `MEMORY_SERVICE — long-term`
- Styled like a database (cylinder shape, heavier/darker fill, or a small database icon)
- Example contents inside: `past complaints`, `patterns`, `...`

**Critical arrow:** A **solid arrow** from STATE down to MEMORY_SERVICE, labeled `"session ends"`. This is the most important visual element on the slide — the entire teaching point in one arrow.

> **Do not** connect MEMORY_SERVICE directly to the orchestrator. The only path is: `STATE → "session ends" → MEMORY_SERVICE`.
...
- Highlight `tool_context.state[...]` in the top block.
- Highlight `memory_service.add(...)` in the bottom block.
- Use **different colors** if possible — emphasizes the parallel-but-not-same shape.
