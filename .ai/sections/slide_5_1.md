...
**Title bar:** `C1 — Non-determinism & evals | distributed systems / testing pyramid`

---

## Diagram

**Top panel (~1/3 of slide):**

```
same input → AGENT → different outputs (but valid)
```

Three example outputs showing the same outcome, different wording:
...
Header above the bottom panel: **evals = tests, probabilistic**

**Bottom panel — inverted testing pyramid (wide at bottom, narrow at top):**

| Layer | Box type | Evaluates |
|---|---|---|
| TRAJECTORY | glass box *(visually emphasized — new layer)* | Did it take the right path? (tools, order, arguments) |
| SINGLE STEP | white box | Was each individual decision correct? |
| FINAL RESPONSE | black box | Did the answer match the rubric? |

TRAJECTORY should have a different fill, thicker border, or a "new" badge — it's the layer that's genuinely new.

## Design Notes

This taxonomy (single step / trajectory / final response) is the standard agent-evaluation taxonomy used by Langfuse, MLflow, Confident AI, LangChain. The "glass box / white box / black box" captions are doing memorable work — give them visual prominence.
...
Middle layer: single-step evals. White-box. You isolate one decision the agent made — say, the moment it chose to call `check_fraud_patterns` — and you grade that decision on its own. Did it pick the right tool? Did it pass the right arguments? This is the closest analogue to a unit test in agent-land.
