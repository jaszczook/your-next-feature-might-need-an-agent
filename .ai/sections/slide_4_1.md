...
**Title bar:** `P1 — Tools | service calls / adapter pattern`

---

## Diagram

One agent box at the top, labeled **CASE_AGENT**. Same visual style as the AGENT in the loop diagram from Section 2.

Four tool boxes in the middle row — rendered as a visually distinct interface layer (smaller boxes, rounded corners, lighter fill — these need to look like "movable pieces," because in P2 they redistribute under sub-agents):
- `get_account`
- `get_transactions`
...
- `get_transactions` → Account Service *(same service — one box with two arrows is cleaner)*
...
Bold/color the entire `tools=[...]` list. The list IS the lesson — this is the agent's complete capability surface.
